...
function Home() {
  ...
  useEffect(() => {
    fetch(`https://newsapi.org/v2/top-headlines?country=br&apiKey=${process.env.REACT_APP_API_KEY}`)
  ...
  useEffect(() => {
    ...
    const newsBeingDisplayed = filteredNews.slice(indexOfFirstNews, indexOfLastNews);
  ...
  function handleSearch(e) {
    ...
    if(searchedWord === '') {
      ...
      const searchedNews = news.filter( article => article.title.toLowerCase().includes(search.toLowerCase()));
